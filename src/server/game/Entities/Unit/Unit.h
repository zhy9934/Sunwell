...
struct CharmInfo
{
    ...
    {
        return _forcedSpellId;
    }
    void SetForcedTargetGUID(uint64 guid)
    {
        _forcedTargetGUID = guid;
    }
    uint64 GetForcedTarget()
    {
        return _forcedTargetGUID;
    }
...
class SafeUnitPointer
{
    ...
    { }
    SafeUnitPointer(const SafeUnitPointer& p)
    {
        ASSERT(false);
    }
    void Initialize(Unit* defVal)
    {
        defaultValue = defVal; ptr = defVal;
    }
    ~SafeUnitPointer();
...
class Unit : public WorldObject
{
    ...
    {
        float reach = m_floatValues [UNIT_FIELD_COMBATREACH]; return reach > MIN_MELEE_REACH ? reach : MIN_MELEE_REACH;
    }
    ...
    {
        return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_MELEE, 2.0f, 100.0f, damage);
    }
    uint32 GetRangedDamageReduction(uint32 damage) const
    {
        return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_RANGED, 2.0f, 100.0f, damage);
    }
    uint32 GetSpellDamageReduction(uint32 damage) const
    {
        return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_SPELL, 2.0f, 100.0f, damage);
    }
    ...
    {
        return HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PET_IN_COMBAT);
    }
    ...
    {
        m_visibleAuras [slot] = aur; UpdateAuraForGroup(slot);
    }
    void RemoveVisibleAura(uint8 slot)
    {
        m_visibleAuras.erase(slot); UpdateAuraForGroup(slot);
    }
    ...
    {
        if (IsSummon()) return reinterpret_cast<const TempSummon*>(this); else return NULL;
    }
